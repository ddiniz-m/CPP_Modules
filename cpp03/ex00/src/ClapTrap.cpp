...
int	ClapTrap::getHp(void)
{
	if (this->hp == 0)
		return (0);
	std::cout << this->getName() << " has " << this->hp << " health points!" << std::endl;
	return(this->hp);
}

int	ClapTrap::getEnergy(void)
{
	std::cout << this->getName() << " has " << this->energy << " energy points!" << std::endl;
	return (this->energy);
}
...
void	ClapTrap::setAttack(unsigned int amount)
{
	...
	this->atk = amount;
	std::cout << this->getName() << " deals " << this->atk << " damage." << std::endl;
}
...
void ClapTrap::beRepaired(unsigned int amount)
{
	...
	if (amount + this->hp > 10)
		...
		this->hp += amount;
	this->energy--;
	std::cout << this->getName() << " repaired " << amount << " health points" << std::endl;
}